...
/**
 * Central class for ratingallocate module.
 *
...
use Exception;
use html_writer;
use manual_alloc_form;
use mod_ratingallocate;
use mod_ratingallocate\choice\ratingallocate_choice;
use mod_ratingallocate\db as this_db;
use mod_ratingallocate\event\all_ratings_deleted;
use mod_ratingallocate\event\allocation_published;
use mod_ratingallocate\event\allocation_statistics_viewed;
use mod_ratingallocate\event\allocation_table_viewed;
use mod_ratingallocate\event\distribution_triggered;
use mod_ratingallocate\event\manual_allocation_saved;
use mod_ratingallocate\event\rating_deleted;
use mod_ratingallocate\event\rating_saved;
use mod_ratingallocate\event\rating_viewed;
use mod_ratingallocate\event\ratingallocate_viewed;
use mod_ratingallocate\event\ratings_and_allocation_table_viewed;
...
class ratingallocate {
    ...
    private function process_action_start_distribution() {
        ...
        if (has_capability('mod/ratingallocate:start_distribution', $this->context)) {
            if ($this->get_algorithm_status() === algorithm_status::RUNNING) {
                ...
                $this->get_algorithm_status() === algorithm_status::NOTSTARTED
            ) {
                ...
                $this->ratingallocate->algorithmstatus = algorithm_status::NOTSTARTED;
                $this->origdbrecord->{this_db\ratingallocate::RUNALGORITHMBYCRON} = '1';
                $this->origdbrecord->{this_db\ratingallocate::ALGORITHMSTATUS} = algorithm_status::NOTSTARTED;
                ...
                $this->clear_distribute_unallocated_tasks();
                $this->origdbrecord->{this_db\ratingallocate::ALGORITHMSTATUS} = algorithm_status::RUNNING;
                $DB->update_record(this_db\ratingallocate::TABLE, $this->origdbrecord);
    ...
    private function process_action_give_rating() {
        ...
        if (has_capability('mod/ratingallocate:give_rating', $this->context, null, false)) {
            ...
            if (!$DB->record_exists('ratingallocate_choices', ['ratingallocateid' => $this->ratingallocateid])) {
                ...
                // Logging.
                $event = rating_viewed::create_simple(
                    context_module::instance($this->coursemodule->id),
                    $this->ratingallocateid
                );
                $event->trigger();
                ...
                if (!$mform->is_cancelled()) {
                    if ($mform->is_validated()) {
                        ...
                        $renderer->add_notification(
                            get_string("choice_added_notification", RATINGALLOCATE_MOD_NAME),
                            self::NOTIFY_SUCCESS
                        );
        ...
        if (has_capability('mod/ratingallocate:start_distribution', $this->context)) {
            ...
            if (!$mform->no_submit_button_pressed() && $data = $mform->get_submitted_data()) {
                if (!$mform->is_cancelled()) {
                    ...
                    $status = $this->get_status();
                    if (
                        $status === self::DISTRIBUTION_STATUS_TOO_EARLY ||
    ...
    private function get_undistributed_users_with_groupscount(): array {
        ...
        $raters = $this->get_raters_in_course();
        $undistributedusers = array_map(fn($user) => $user->id, array_values(array_filter(
            $raters,
    ...
    private function process_action_show_ratings_and_alloc_table() {
        ...
        if (has_capability('mod/ratingallocate:start_distribution', $this->context)) {
            ...
            // Logging.
            $event = ratings_and_allocation_table_viewed::create_simple(
                context_module::instance($this->coursemodule->id),
                $this->ratingallocateid
            );
            $event->trigger();
    ...
    private function process_action_show_allocation_table() {
        ...
        if (has_capability('mod/ratingallocate:start_distribution', $this->context)) {
            ...
            // Logging.
            $event = allocation_table_viewed::create_simple(
                context_module::instance($this->coursemodule->id),
                $this->ratingallocateid
            );
            $event->trigger();
    ...
    private function process_action_show_statistics() {
        ...
        if (has_capability('mod/ratingallocate:start_distribution', $this->context)) {
            ...
            // Logging.
            $event = allocation_statistics_viewed::create_simple(
                context_module::instance($this->coursemodule->id),
                $this->ratingallocateid
            );
            $event->trigger();
    ...
    private function process_default() {
        ...
        // Logging.
        $event = ratingallocate_viewed::create_simple(
            context_module::instance($this->coursemodule->id),
            $this->ratingallocateid
        );
        $event->trigger();
    ...
    public function distribute_choices() {
        ...
        $this->origdbrecord->algorithmstatus = algorithm_status::RUNNING;
        ...
        $this->origdbrecord->algorithmstatus = algorithm_status::FINISHED;
        $this->db->update_record(this_db\ratingallocate::TABLE, $this->origdbrecord);
    ...
    public function publish_allocation() {
        ...
        // Logging.
        $event = allocation_published::create_simple(
            context_module::instance($this->coursemodule->id),
            $this->ratingallocateid
        );
        $event->trigger();
    ...
    public function set_algorithm_failed() {
        $this->origdbrecord->algorithmstatus = algorithm_status::FAILURE;
        $this->db->update_record(this_db\ratingallocate::TABLE, $this->origdbrecord);
    }
    ...
    public function notify_users_distribution() {
        ...
        foreach ($users as $userid => $allocobj) {
            ...
            $notificationsubject = format_string($this->course->shortname, true) . ': ' .
                get_string(
                    'allocation_notification_message_subject',
                    'ratingallocate',
    ...
    public function delete_all_ratings() {
        ...
        try {
            ...
            // Logging.
            $event = all_ratings_deleted::create_simple(
                context_module::instance($this->coursemodule->id),
                $this->ratingallocateid
            );
            $event->trigger();
    ...
    public function delete_ratings_of_user($userid) {
        ...
        try {
            ...
            // Logging.
            $event = rating_deleted::create_simple(
                context_module::instance($this->coursemodule->id),
                $this->ratingallocateid
            );
            $event->trigger();
    ...
    public function save_ratings_to_db($userid, array $data) {
        ...
        try {
            ...
            // Logging.
            $event = rating_saved::create_simple(
                context_module::instance($this->coursemodule->id),
                $this->ratingallocateid,
                $loggingdata
            );
    ...
    public function filter_choices_by_groups($choices, $userid) {
        ...
        if (
            has_capability('mod/ratingallocate:modify_choices', $this->context)
    ...
    public function save_manual_allocation_form($allocdata, $userdata) {
        try {
            ...
            // Logging.
            $event = manual_allocation_saved::create_simple(
                context_module::instance($this->coursemodule->id),
                $this->ratingallocateid
            );
            $event->trigger();
    ...
    public function alter_allocation($oldchoiceid, $newchoiceid, $userid): bool {
        $this->db->set_field(
            ...
            this_db\ratingallocate_allocations::CHOICEID,
            $newchoiceid,
            [
    ...
    public function clear_distribute_unallocated_tasks(): bool {
        global $DB;
        $queuedtasks = manager::get_adhoc_tasks(distribute_unallocated_task::class);
